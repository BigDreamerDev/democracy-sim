...
(function () {
  ...
  async function viewDiplomacy() {
    ...
    box.innerHTML = `<div class="diplomacy-office">
      ...
      ${
        ...
        <div class="card"><h3 style="margin-top:0">All foreign powers</h3><p class="small muted">Runs every active power's government turn in sequence — powers with no government configured are skipped, not failed.</p><label class="field" style="margin-bottom:10px"><span><input type="checkbox" id="ro-run-all-force" style="width:auto;margin-right:6px">Force a fresh turn even if one already ran this cycle</span></label><button class="btn btn-primary" id="ro-run-all-turns">Run all foreign turns</button><div id="ro-run-all-progress"></div><div id="ro-run-all-result"></div></div>
    ...
    async function loadRoPower(id) {
      ...
      try {
        ...
        document.querySelector('#ro-run-turn').onclick = async () => {
          try {
            const force = document.querySelector('#ro-run-force')?.checked === true;
            const r = await api(`/api/admin/foreign/powers/${id}/run-turn`, { method: 'POST', body: { force } });
            toast(
              r.already_ran
    ...
    if (runAll) {
      runAll.onclick = () => busy(runAll, async () => {
        ...
        try {
          const force = document.querySelector('#ro-run-all-force')?.checked === true;
          await apiStream('/api/admin/foreign/run-all-turns', { method: 'POST', body: { force } }, line => {
            if (line.type === 'total') {
              total = line.count;
              renderBar(0, null);
  ...
  async function viewIntel(v, recentOperation = null) {
    ...
    const [assets, citizens, assignments, agents, compromisedAgents, leaks] = await Promise.all([
      ...
      service ? api('/api/intel/leaks').catch(() => []) : Promise.resolve([])
    ]);
    ...
    v.innerHTML = `
      ...
      ${cleared && service ? `<div class="card"><h2>${iAmDirector ? 'Operation assignments' : 'Your operation assignments'}</h2><p class="small muted">Assignment identities are Service information and do not appear in the public operations register.</p><div class="list">${assignmentRows}</div></div><div class="card"><h2>Reports</h2><p class="small muted">A sealed report opens only to a cleared citizen. Reading it is itself logged and public. A cleared citizen may also leak a sealed report early — that too is logged and public, and attributed by name.</p><div class="list">${reportRows || '<div class="empty">No reports have been filed.</div>'}</div></div>` : ''}

      ${service ? `<div class="card"><h2>Leak register</h2><p class="small muted">Public and permanent: who leaked which report, and when. A leak is attributed, not anonymous.</p>${leakRows}</div>` : ''}
    ...
    document.querySelectorAll('[data-intel-leak]').forEach(btn => btn.onclick = () => busy(btn, async () => {
      if (!confirm('Leak this report now? This is attributed to your name on the public leak register and cannot be undone.')) return;
      try {
        await api(`/api/intel/reports/${btn.dataset.intelLeak}/leak`, { method: 'POST' });
        toast('Report leaked. It is public, and the leak is attributed to you on the record.');
        await viewIntel(v);
      } catch (err) { toast(err.message, true); }
    }));
  }
